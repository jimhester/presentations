...
- Most proposals have working implementations (click links)
...
- Allows user defined `%foo%` in unary contexts.
...
head(mtcars) %g% "{name} has {hp} hp"
```

# [Native pipe operator](https://github.com/wch/r-source/compare/trunk...jimhester:feature-pipe_operator)
...
- Consistent use of `_` as the pipe placeholder.
...
# [Compact anonymous functions](https://github.com/jimhester/lambda)
...
# [Triple-Quoted String Literals](https://cran.r-project.org/package=glue)
```julia
str = """
         Hello,
           world.
...
dbExecute(con, "SET @t1 = 1, @t2 = 2, @t3 = 4")
dbGetQuery(con, "SELECT @t1, @t2, @t3, @t4 := @t1 + @t2 + @t3")
```
...
- [Julia Variables](https://docs.julialang.org/en/release-0.4/manual/variables/)
  - Tab completions with LaTeX-like symbol names
    - `delta-<tab>` -> `δ`
